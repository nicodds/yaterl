...
task :test => [:compile] do
  sh("erl -noshell -s ct_run script_start -s erlang halt -pa $PWD/OUTPUT/ebin-codecov #{prepare_commontest.chomp}")
end

task :test_vts => [:compile] do
  sh("erl -noshell -s webtool script_start vts chromium-browser -s ct_run script_start -pa $PWD/OUTPUT/ebin-codecov #{prepare_commontest.chomp}")
end
